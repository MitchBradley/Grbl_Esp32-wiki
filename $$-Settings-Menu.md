...
To view the settings, type `$$` and press enter after connecting.  It should respond with a list of the current system settings  (see below). All of these settings are persistent. If you power down, these will be loaded back up the next time you power up. The settings are only accessible in the idle state.

Grbl_ESP32 has more available settings than Arduino Grbl. To prevent this extended list from possibly confusing GUI senders, the default is to only show the basic list. There are two ways to see the extended list. You can uncomment **#define SHOW_EXTENDED_SETTINGS** in config.h. This will show them when you send the **$$** command. The other way is to send the Grbl_ESP32 **$+** command.

The `x` of `$x=val` indicates a particular setting, while `val` is the setting value. You send $x=val to change the value.
...
## $x=val` settings and what they mean
...
#### $35 - Spindle PWM Min (extended setting)

This is the PWM minimum value as a percentage. This is typically left at the default of 0. On some lasers and spindles they will not com on until some minimum PWM is reached

#### $36 - Spindle PWM Max  (extended setting)

This is the PWM maximum value as a percentage. This is typically left at the default of 100.0

#### $80 thru $84  - Integer values for custom machines  (extended setting)

These are values reserved for people creating custom machines.

#### $90 thru $94 - Floating point values for custom machines  (extended setting)
...
#### $110 thru $116 – [X,Y,Z,A,B,C] Max rate, mm/min
...
#### $120 thru $125 – [X,Y,Z,A,B,C] Acceleration, mm/sec^2
...
#### $130 thru $135 – [X,Y,Z,A,B,C] Max travel, mm
...
#### $140 thru $145 – [X,Y,Z,A,B,C]  Stepper Run Current (extended settings)

This is for Trinamic SPI stepper drivers only

#### $150 thru $155 – [X,Y,Z,A,B,C]  Stepper Idle Current (extended settings)

This is for Trinamic SPI stepper drivers only

#### $160 thru $165 – [X,Y,Z,A,B,C]  Microstepping (extended settings)

This is for Trinamic SPI stepper drivers only

#### $170 thru $175 – [X,Y,Z,A,B,C]  Stall Guard Value (extended settings)
...
The default values are in the defaults.h file. You can override any of the defaults by placing one or more of them in your CPU map.